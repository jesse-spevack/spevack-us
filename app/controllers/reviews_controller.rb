class ReviewsController < ApplicationController
  ...
  def show
    ...
    Rails.logger.error "Date.current: #{Date.current}"
    Rails.logger.error "Date.current.beginning_of_week: #{Date.current.beginning_of_week}"
    Rails.logger.error "Date.current.beginning_of_week - 7.days: #{Date.current.beginning_of_week - 7.days}"
